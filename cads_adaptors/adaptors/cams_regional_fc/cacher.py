...
class CacherS3(AbstractAsyncCacher):
    ...
    def _write_field_sync(self, data, fieldinfo):
        ...
        remote_path = ('permanent'
                       if self._cache_permanently(fieldinfo) else
                       'temporary') + '/' + self._cache_file_path(fieldinfo)
        ...
        while True:
            ...
            try:
                ...
                self.context.error('Failed to upload to S3 bucket (attempt '
                                   f'#{attempt}): {exc!r}')
    ...
    def cache_file_url(self, fieldinfo):
        """Return the URL of the specified field in the cache"""
        return f'https://{self._host}/{self._bucket}/' + \
