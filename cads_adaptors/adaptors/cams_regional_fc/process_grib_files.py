...
from eccodes import codes_get_message, codes_release, codes_get, codes_set

from cds_common.message_iterators import grib_file_iterator
from cds_common.message_iterators import grib_file_ordered_iterator
...
def data_processor(req_group, alterations, context):
    ...
    if not any(alterations.values()):
        ...
        if alterations['ordered']:
            iterator = grib_file_ordered_iterator(req_group['retrieved_files'],
                                                  req_group['requests'],
                                                  grib2request,
                                                  context)
        else:
        ...
        for msg in iterator:
            ...
            if alterations['surface_fix'] and \
               codes_get(msg, 'typeOfLevel' ) == 'surface':
