...
from cds_common import hcube_tools, date_tools
...
def set_area(request, area_list, grid, context):
    ...
    area_list = [(v if v is not None else d)
                 for v, d in zip(area_list, default_area.values())]
    ...
    if area['north'] <= area['south']:
        raise BadRequestException('area north limit must be greater than ' +
                                  'south limit', '')
    if area['east'] <= area['west']:
        raise BadRequestException('area east limit must be greater than ' +
                                  'west limit', '')

    # Snap to the grid
    area['north'] = snap_to_grid(area['north'], grid['south'], grid['dlat'],
                                 floor)
    area['west']  = snap_to_grid(area['west'],  grid['west'],  grid['dlon'],
                                 ceil)
    area['south'] = snap_to_grid(area['south'], grid['south'], grid['dlat'],
                                 ceil)
    area['east']  = snap_to_grid(area['east'],  grid['west'],  grid['dlon'],
                                 floor)
    if area['north'] < area['south'] or area['east'] < area['west']:
        raise BadRequestException('requested area does not contain a ' +
                                  'grid point', '')

    # Only insert area in request if it's not the full area (for caching
    # reasons)
    request_out = deepcopy(request)
    for k, incr in [('north', -grid['dlat'] / 2.),
                    ('south',  grid['dlat'] / 2.),
                    ('east',  -grid['dlon'] / 2.),
                    ('west',   grid['dlon'] / 2.)]:
        ...
        if area[k] * direction > (grid[k] + incr) * direction:
            raise BadRequestException(
                'Area ' + k + ' value lies outside model grid limit of ' +
                str(grid[k]) + ' for date(s)=' + repr(request['date']), '')
...
def model_grids_table(grids, regapi):
    ...
    for grid, requested in grids.items():
        ...
        # in available
        available_keys = sum([list(hcube.keys()) for hcube in available],
                             ['date'])
        requested = [{k: v for k, v in r.items() if k in available_keys}
                     for r in requested]
        hcube_tools.hcubes_merge(requested)

        # Make nice strings to show the area, resolution and requested fields
        grid = dict(grid)
        area = '[' + ','.join(['{:g}'.format(grid[k])
                               for k in ['north', 'west', 'south', 'east']]) + \
               ']'
        resn = '[' + ','.join('{:g}'.format(grid[k])
                              for k in ['dlon', 'dlat']) + \
               ']'
        requested_strings = []
        for r in requested:
    ...
    return tabulate(strings,
                    headers=['Area [N,W,S,E]',
...
def locally_extracted_area(requests):
    ...
    if local_subarea:
        ...
        for r in requests:
            if 'north' in r:
                areas.add(tuple([r.pop(k)[0] for k in
