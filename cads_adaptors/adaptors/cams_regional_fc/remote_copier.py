...
class RemoteCopier:
    ...
    def __init__(self, logger=logging.getLogger(__name__)):
        ...
        self._unique_string = datetime.now().strftime('%s%f') + '.' + \
                              str(rand)
        self._ssh_opts = ['-o', 'BatchMode=yes',
                          '-o', 'StrictHostKeyChecking=no',
                          '-o', 'ConnectTimeout=2',
                          '-o', 'ControlPath=' +
                                '~/.ssh/master-%C.' + str(rand),
        ...
        self._temp_dirs = {'local': '/cache/tmp',
                           'compute-.*': '/cache/tmp',
    ...
    def _get_tar_file(self, user, host):
        ...
        if uhost not in self._tf:
            ...
            name = 'regional_fc.{}.{}.{}'.format(uhost,
                                                 self._unique_string,
                                                 self._tf_count[uhost])
            tf = {'path': self._temp_dirs['local'] + f'/{name}.l.tar',
    ...
    def _copy_tar(self, uhost):
        ...
        if tf is None:
            ...
            return
        tf['fileobj'].close()
        self._logger.info(f'Copying tar file containing {tf["nfiles"]}'
                          f' members to {uhost}')
        self._exec(['scp'] + self._ssh_opts + \
                   [tf['path'], f'{uhost}:{tf["remote_path"]}'])
    ...
    def _exec(self, cmd, **kwargs):
        ...
        if proc.stdout:
            self._logger.info('stdout from ' + repr(cmd) + ': ' +
                               str(proc.stdout))
        if proc.stderr:
            self._logger.warning('stderr from ' + repr(cmd) + ': ' +
                                 str(proc.stderr))
    ...
    with RemoteCopier() as r:
        for i in range(100):
            ...
            with open(file, 'rb') as f:
                r.copy(f.read(), 'cds', 'compute-0001',
                       '/cache/downloads/' + file)
