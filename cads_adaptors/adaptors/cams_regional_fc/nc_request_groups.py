...
def nc_request_groups(context, reqs, info):
    ...
    context.info(
        "For NetCDF conversion requests have been " + "split/grouped as following..."
    )
    ...
    for req in reqs:
        for dtime in [
            ...
            if dtime not in vtimes:
                vtimes[dtime] = set()
            vtimes[dtime].update([dtime + timedelta(hours=int(s)) for s in req["step"]])
