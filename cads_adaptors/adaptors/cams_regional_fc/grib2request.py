...
def grib2request_init(dataset_dir):
    ...
    field_data.update(
        {'variable': {d['backend_api_name']: d
                      for d in regfc_defns['variable']},
         'model': {d['backend_api_name']: d
                   for d in regfc_defns['model']},
         'type': {
             'FORECAST': {
            ...
            for grib_defn in value_info['grib_representations']:
                for grib_key, grib_value in grib_defn.items():
                    ...
                    if grib_key not in grib_key_types:
                        ...
                        raise Exception('All values for a given grib key ' +
                                        'should have same type: ' + grib_key)
...
def grib2request(msg):
    ...
    if not field_data:
        raise Exception('You must call the initialisation function before this '
                        'function')
    ...
    for grib_key, ktype in grib_key_types.items():
        try:
            ...
            raise Exception('Failed to get grib key "' + grib_key + '": ' +
                            str(e))
    ...
    for api_key, key_info in field_data.items():
        ...
        if api_key not in request:
            ...
            raise Exception('Field does not match any known ADS ' + api_key +
                            ': ' + repr(fld))

    # Set the date and time
    date = fld['dataDate']
    time = fld['dataTime']
    request['date'] = '{}-{:02d}-{:02d}'.format(date // 10000, 
                                                (date // 100) % 100,
                                                date % 100)
...
if __name__ == '__main__':
    ...
    grib2request_init('/home/nal/cds/cds-forms-cams/'
