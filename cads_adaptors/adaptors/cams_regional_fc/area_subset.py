...
from eccodes import codes_grib_new_from_file, codes_get_long, codes_set
from eccodes import codes_get_array, codes_set_array, codes_clone
from eccodes import codes_write, codes_release
...
def area_subset_handle(hndl, area):
    ...
    grid_limits = [snap(codes_get_long(hndl, 'latitudeOfFirstGridPoint')),
    ...
    grid_incr = [codes_get_long(hndl, 'jDirectionIncrement'),
                 codes_get_long(hndl, 'iDirectionIncrement')]
    grid_size = [codes_get_long(hndl, 'Nj'),
                 codes_get_long(hndl, 'Ni')]
    ...
    lats = [irow * grid_incr[0] * lat_dirn + grid_limits[0]
            for irow in range(grid_size[0])]
    lons = [shift_lon(icol * grid_incr[1] + grid_limits[1], -180000000)
    ...
    for iv, value in enumerate(codes_get_array(hndl, 'values')):
        ...
        if latmd >= area_md[2] and latmd <= area_md[0] and \
           lonmd >= area_md[1] and lonmd <= area_md[3]:
            values.append(value)
    if len(values) != npnts_new:
        raise Exception('Did not extract expected number of points: ' +
...
if __name__ == '__main__':
    area_subset([70., 0, 69.95, 0.05], 'one_field.grib', 'a.grib')
