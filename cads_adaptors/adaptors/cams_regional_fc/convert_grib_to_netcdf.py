...
def convert_grib_to_netcdf(requests, gribfile, ncfile, regfc_defns):
    ...
    envelope['species'] = [vdefns[var]
                           for var in envelope['request']['variable']]

    # Compute a list of all requested validity times
    envelope['dates'] = sorted([datetime.strptime(d, '%Y-%m-%d')
                                for d in envelope['request']['date']])
    for t in envelope['request']['time']:
        assert len(t) == 4
    envelope['vtimes'] = sorted(
        [d + timedelta(hours= int(t[0:2]) + int(t[2:4]) / 60. + int(s))
         for d, t, s in product(envelope['dates'],
    ...
    envelope['levels'] = sorted([float(x) for x in 
                                 envelope['request']['level']])
...
def _convert_grib_to_netcdf(envelope, gribfile, nc, regfc_defns):
    ...
    keys2read = ['parameterNumber', 'constituentType',
                 'productDefinitionTemplateNumber',
                 ('dataDate', str), ('dataTime', str), 
                 'typeOfProcessedData', 'forecastTime', 'level', 'Ni', 'Nj',
    ...
    first = True
    const_hdr = None
    written = np.zeros((len(envelope['vtimes']), len(envelope['levels']),
                        len(envelope['species'])),
                       dtype=bool)
    for msg in grib_file_iterator(gribfile):
        ...
        if const_hdr is None:
            ...
            raise Exception('Some of these grib keys are not the same for ' +
                            'every field: ' + repr(const_hdr.keys()) +
                            '\n    First field: ' + repr(const_hdr) +
                            '\n    This field: ' + repr(const))
        ...
        try:
            ...
            raise Exception('Encountered exception when processing grib ' +
                            'field ' + repr(hdr) + ': ' + repr(e))
...
def write_msg(msg, hdr, envelope, nc, written):
    ...
    vtime = datetime.strptime(hdr['dataDate'] + ' ' + hdr['dataTime'],
                              '%Y%m%d %H%M') \
...
def envelope_request(requests):
    ...
    def keyset(req):
        return set([k for k in req.keys()
                    if k not in ['north', 'south', 'east', 'west']])
    keys = keyset(requests[0])
    envelope_req = {k: set() for k in keys}
    for req in requests:
        if keyset(req) != keys:
            raise Exception('Input requests do not all have same keys: ' +
                            repr(keys) + ' ' + repr(keyset(req)))
        for k in keys:
...
def ncinit(envelope, nc, hdr, regfc_defns):
    ...
    for species in envelope['species']:
        atts = species['netcdf']
        vdata = nc.createVariable(atts['varname'], 'f4',
    ...
    vtime[:] = [(t - vbasetime).total_seconds()/3600.
                for t in envelope['vtimes']]
    vlev[:] = envelope['levels']

    # Compute grid latitudes
    if hdr['Nj'] > 1:
        dlat = (hdr['latitudeOfLastGridPointInDegrees'] -
                hdr['latitudeOfFirstGridPointInDegrees']) \
            / (hdr['Nj'] - 1)
    else:
        dlat = 0
    lats = np.array([hdr['latitudeOfFirstGridPointInDegrees'] + i * dlat
                     for i in range(hdr['Nj'])])

    # Compute grid longitudes
    while hdr['longitudeOfFirstGridPointInDegrees'] > \
          hdr['longitudeOfLastGridPointInDegrees']:
        hdr['longitudeOfFirstGridPointInDegrees'] -= 360.
    if hdr['Ni'] > 1:
        dlon = (hdr['longitudeOfLastGridPointInDegrees'] -
                hdr['longitudeOfFirstGridPointInDegrees']) \
            / (hdr['Ni'] - 1)
    else:
        dlon = 0
    lons = np.array([hdr['longitudeOfFirstGridPointInDegrees'] + i * dlon
                     for i in range(hdr['Ni'])])
    # Ensure lons are in the range 0 to 360
    lons = lons - np.floor(lons / 360.) * 360.
...
def set_globatts(nc, envelope, typename, regfc_defns):
    ...
    if len(envelope['dates']) == 1:
        ...
        date_string = '-'.join([envelope['dates'][i].strftime('%Y%m%d')
                                for i in [0, -1]])
    if typename == 'ANALYSIS':
        times = [int(x[0:2]) for x in envreq['time']]
        time_string = '{}+[{}H_{}H]'.format(date_string,
                                            min(times), max(times))
    ...
    shortnames = '/'.join([species['netcdf']['shortname']
                           for species in envelope['species']])
    title_template = '{species} Air Pollutant {fc_an} at {levels}'
    if replicate_flaws and envreq['level'] != ['0']:
        title_template += ': '
    nc.title = title_template.format(species=shortnames,
                                     fc_an=typename,
                                     levels=levstr)
    ...
    if typename == 'ANALYSIS':
        ...
        raise Exception('Unrecognised type: ' + typename)
    nc.summary = ('{modelname} model hourly {fc_an} of {species} ' +
                  'concentration at {levels} from {date_steps} on ' +
                  'Europe').format(modelname=model_name2,
                                   fc_an=typename,
                                   species=shortnames,
                                   levels=levstr,
