...
def api_retrieve(context, requests, regapi, dataset_dir,
                 no_cache_put=False, **kwargs):
    ...
    backend_specific = {
        ...
        'archived': {
            ...
            'max_attempts': {
                'default': lambda req: (req['total_duration_secs'] < 10 and
                                        req['attempt'] < 10),
                'exception':
        ...
        'latest': {
            ...
            'max_attempts': {
                'default': 50,
                404: 1,
                400: 10},    # 400's can get raised at 2am
    ...
    urlreqs = list(requests_to_urls(requests,
                                    regapi.url_patterns))
    ...
    with Cacher(context, no_put=no_cache_put) as cacher:
        ...
        context.info(f'Attempted download of {nfields} fields took ' +
...
def make_api_hypercubes(requests, regapi, context):
    ...
    def levgroups(levels):
        return [g for g in [[l for l in levels if l == '0'],
                            [l for l in levels if l != '0']]
                if g]
    output = []
    for request in requests:
        ...
        for r1 in sum(regapi.split_request_by_grid(request).values(), []):
            ...
            for model, xtype, variable, levels in \
                product(r1['model'],
                        r1['type'],
                        r1['variable'],
                        levgroups(r1['level'])):
                ...
                for r3 in hcube_tools.hcubes_split(r2, 50):
                    ...
                    output.append({k: ','.join([str(x) for x in v])
                                   for k, v in r3.items()})
...
class CamsRegionalFcApiLimiter():
    ...
    def __init__(self, regapi):
        ...
        self._rate_semaphores = {k: Semaphore()
                                 for k in self._max_rate.keys()}
        self._number_semaphores = {k: Semaphore(v) 
                                   for k, v in self._max_simultaneous.items()}
...
class CamsRegionalFcApiRateLimiter(CamsRegionalFcApiLimiter):
    ...
    def block(self, req):
        ...
        Timer(1 / self._max_rate[type],
              self._rate_semaphores[type].release).start()
