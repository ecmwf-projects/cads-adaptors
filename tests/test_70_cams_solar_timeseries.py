...
def test_user_id_normal():
    ...
    for cfg in [{},
        ...
        for req in [{}, {"_user_id": "downstream_user_id"}]:
            ...
            assert username == "72bd0fb92fc1f6fbcbc53cb83d98d7e0df0bd0a833d0b8d296fe4ac6cfb63b39"
...
def test_user_id_service():
    ...
    adp = CamsSolarRadiationTimeseriesAdaptor(
        form=[],
        user_uid="foobar",
        downstream_services={"wekeo": "foobar"}
    )
    ...
    assert username == "347d6ccec325d104fd800e9c47f23337e804655a6b31d07a6b3ccb88b1663523"
    assert verify(username)
    ...
    username = encode(adp._user_id(request))
    assert username == "wekeo_77fb31d9c04bb7c6118c4de41968bc82a84157ef21a7852759fe61155717df30"
    assert verify(username)
...
def test_user_id_service_multi():
    ...
    for ads_user in wekeo_ads_user_ids:
        ...
        adp = CamsSolarRadiationTimeseriesAdaptor(
            form=[],
            user_uid=ads_user,
            downstream_services={"wekeo": wekeo_ads_user_ids,
                                 "mondas": ["wibble"]}
        )
